...
export function BenchUploadForm({ previousId, addAngle }: BenchUploadFormProps) {
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      router.push(
        `/analyze/bench/processing/${analysisId}${qsStr ? `?${qsStr}` : ""}`,
      );
    },
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          type="text"
          autoComplete="off"
          placeholder="e.g. Bodyweight, 225 lbs, 100 kg"
          value={weight}
