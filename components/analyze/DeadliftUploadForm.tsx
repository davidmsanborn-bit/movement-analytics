...
export function DeadliftUploadForm({
  previousId,
  addAngle,
}: DeadliftUploadFormProps) {
  ...
  const onSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      router.push(
        `/analyze/deadlift/processing/${analysisId}${qsStr ? `?${qsStr}` : ""}`,
      );
    },
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          type="text"
          autoComplete="off"
          placeholder="e.g. Bodyweight, 315 lbs, 140 kg"
          value={weight}
