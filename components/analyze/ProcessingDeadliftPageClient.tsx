...
export function ProcessingDeadliftPageClient({ id, previousId, addAngle }: Props) {
  ...
  useEffect(() => {
    ...
    const startUpload = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/analyze-deadlift", {
          method: "POST",
        ...
        console.log("[processing] POST /api/analyze-deadlift response", {
          analysisId: id,
          status: res.status,
          ok: res.ok,
    ...
    const runStatusPoll = async () => {
      ...
      try {
        const res = await fetch(`/api/status-deadlift/${id}`);
