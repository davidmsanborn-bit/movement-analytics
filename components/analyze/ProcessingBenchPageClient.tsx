...
export function ProcessingBenchPageClient({ id, previousId, addAngle }: Props) {
  ...
  useEffect(() => {
    ...
    const startUpload = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/analyze-bench", {
          method: "POST",
        ...
        console.log("[processing] POST /api/analyze-bench response", {
          analysisId: id,
          status: res.status,
          ok: res.ok,
    ...
    const runStatusPoll = async () => {
      ...
      try {
        const res = await fetch(`/api/status-bench/${id}`);
        ...
        if (res.status === 200 && data.status === "ready") {
          ...
          const resultsQs =
            previousId && isValidAnalysisId(previousId)
              ...
              : "";
          router.push(`/results/bench/${id}${resultsQs}`);
          return;
        }
