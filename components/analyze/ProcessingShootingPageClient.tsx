...
export function ProcessingShootingPageClient({
  id,
  previousId,
  addAngle,
}: Props) {
  ...
  useEffect(() => {
    ...
    const startUpload = async () => {
      ...
      try {
        ...
        console.log("[processing] POST /api/analyze-shooting response", {
          analysisId: id,
          status: res.status,
          ok: res.ok,
    ...
    const runStatusPoll = async () => {
      ...
      try {
        const res = await fetch(`/api/status-shooting/${id}`);
        ...
        if (res.status === 200 && data.status === "ready") {
          ...
          const resultsQs =
            previousId && isValidAnalysisId(previousId)
              ...
              : "";
          router.push(`/results/shooting/${id}${resultsQs}`);
          return;
        }
