...
import { DeadliftScoreComparison } from "@/components/results/DeadliftScoreComparison";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Deadlift assessment — ${id.slice(0, 8)}…`,
    description: "Deadlift movement quality assessment from your uploaded video.",
  };
}

export default async function DeadliftResultsPage({ params, searchParams }: Props) {
  const { id } = await params;
  if (!isValidAnalysisId(id)) {
    notFound();
  }
  ...
  let previousAnalysis: DeadliftAnalysisResult | null = null;
  if (
    previousIdParam &&
    isValidAnalysisId(previousIdParam) &&
    previousIdParam !== id
  ) {
    previousAnalysis = await fetchDeadliftAnalysis(previousIdParam);
  }
    ...
    <main className="min-h-full pb-24 pt-12 md:pt-16">
      <PageSection className="space-y-12">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <Link
            href={`/analyze/deadlift?previousId=${encodeURIComponent(result.id)}`}
        ...
        {previousAnalysis ? (
          <DeadliftScoreComparison previous={previousAnalysis} current={result} />
        ) : null}
        <ResultsHeader
          ...
          id={result.id}
          addAngleHref={`/analyze/deadlift?previousId=${encodeURIComponent(result.id)}&addAngle=true`}
        />
        <section className="grid gap-6 lg:grid-cols-2 lg:items-start">
          ...
          />
          <CoachingCues cues={result.coachingCues} sport="deadlift" mode="compact" />
        </section>

        <SubScoreGrid
          subScores={result.subScores}
          description="Five dimensions from your clip. Scores describe quality ranges, not a single “perfect” pull."
        />
        ...
        <DisclaimerStrip />
        <Link
          href="/analyze/deadlift?sessionContinue=true"
