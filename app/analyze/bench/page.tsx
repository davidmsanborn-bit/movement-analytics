import { BenchUploadForm } from "@/components/analyze/BenchUploadForm";
...
export const metadata: Metadata = {
  title: "Analyze bench press — Movement Analytics",
  description:
    "Upload a bench press clip for movement quality assessment.",
};
...
export default async function AnalyzeBenchPage({ searchParams }: PageProps) {
  ...
  const previousResult = validPreviousId
    ? await fetchBenchAnalysis(validPreviousId)
    : null;
    ...
    <main className="min-h-full bg-[var(--bg-page)] pb-24 pt-12 md:pt-16">
      <PageSection>
        ...
        <p className="font-mono text-xs font-medium uppercase tracking-[0.2em] text-[var(--accent)]">
          Bench press · Side or spotter view
        </p>
        ...
        <p className="mt-4 max-w-2xl text-[var(--text-secondary)]">
          {addAngle
            ...
            : "Upload a clip of your bench press — barbell, dumbbell, or any variation"}
        </p>
        <div className="mt-12 grid gap-10 lg:grid-cols-2">
          <FilmingGuidelines
            angleTip="Side view or spotter view works best — side shows bar path and arch, spotter shows symmetry"
          />
